...
# Define depth of each cell within columns
baseline_length = int(1000/downsampling_factor) # 1s
stim_length = int(5000/downsampling_factor) # 5s
rest_length = int(10000/downsampling_factor) # 10s
imag_length = int(6000/downsampling_factor) # 6s
pupil_length = baseline_length + stim_length + rest_length + imag_length  
...
folder_edf = 'data_repli' # where the .edf datafiles are
...
def get_data(full_path):
    ...
    print(dm.ptrace_baseline.depth, dm.ptrace_imag.depth, dm.ptrace_rest.depth, dm.ptrace_stim.depth)
    print(f"Only the replication version: {dm.exp_name.unique[0] == 'pupillo_repli'}")
    
    # Set depth of each column
    dm.ptrace_baseline.depth = 1000 # 1s
    dm.ptrace_stim.depth = 5000 # 5s
    dm.ptrace_rest.depth = 10000 # 10s
    dm.ptrace_imag.depth = 6000 # 6s
...
def preprocess_df(df):
    ...
    for a in list(df.Q00PARTICIPANT[df.QMI_visual<=3]):
...
def preprocess(dm_to_process):
    ...
    dm.z_pupil = NAN
    for s, sdm in ops.split(dm.subject_id):
        dm.z_pupil[sdm] = ops.z(srs.reduce(sdm.pupil[:, 50:baseline_length])) # last 500ms of the baseline period
    ...
    for p, t, sdm in ops.split(dm.subject_id, dm.trialid):
        dm.mean_baseline[sdm] = srs.reduce(sdm.pupil[:, 0:baseline_length])
    
    # Remove the baseline from the pupil size
    dm.pupil = srs.baseline(
        dm.pupil,
        dm.pupil[:,0:baseline_length],
        int(baseline_length-(factor/2)), int(baseline_length),
        method='subtractive')
    
    # Compute mean pupil size during imagery
    dm.mean_pupil, dm.mean_perception, dm.mean_rest = NAN, NAN, NAN
    for p, t, sdm in ops.split(dm.subject_id, dm.trialid):
        dm.mean_pupil[sdm] = srs.reduce(sdm.pupil[:, pupil_length-imag_length+100:])
        dm.mean_perception[sdm] = srs.reduce(sdm.pupil[:, baseline_length:baseline_length+stim_length:])
        dm.mean_rest[sdm] = srs.reduce(sdm.pupil[:, baseline_length+stim_length:baseline_length+stim_length+rest_length])
...
def plot_dm(dm_to_plot, grouped=True, all_plots=True, title=None):
    ...
    plt.xticks(range(0,22,1), labels=None)
    plt.axvline(baseline_length/factor, color='black', linestyle=':') # Add vertical line for end of baseline/start of stim
    plt.axvline((baseline_length + stim_length)/factor, color='black', linestyle='dotted') # Add vertical line for end of stim/start of rest
    plt.axvline((baseline_length + stim_length + rest_length)/factor, color='black', linestyle='dotted') # Add vertical line for end of rest/start of imagery
    plt.axvline((baseline_length + stim_length + rest_length)/factor + 1, color='black', linestyle='dotted') # Add vertical line for end of rest/start of imagery
    ...
    for stim_color, _dm in ops.split(dm.stim_color): # Split conditions white vs black
        plot.trace(
            ...
            err='se')
        # Add annotations
    plt.annotate('Fixation', rotation=90, xy=(95, 270), xycoords='figure points')
    plt.annotate('Perception', xy=(120, 300), xycoords='figure points')
    plt.annotate('Rest', xy=(245, 300), xycoords='figure points')
    plt.annotate('Imagery', xy=(515, 300), xycoords='figure points')
    ...
    if all_plots==True:
        ...
        for stim_color, _dm in ops.split(dm.stim_color): # Split conditions white vs black
            plot.trace(
                _dm.pupil[:, pupil_length-imag_length+factor:],
                x=np.linspace(0, (imag_length-factor)/factor, imag_length-factor),
                color=red[1] if stim_color == 'bright' else blue[1],
                label=f'{stim_color.title()} (N={len(_dm)})',
                err='se')
        plt.xticks(range(0, 6, 1), labels=range(17, 23, 1), fontsize=18);plt.yticks(fontsize=18)
        plt.axhline(0, linestyle='dotted', color='black')
        plt.xlim([0, (imag_length-factor)/factor])
        plt.show()
        ...
        plt.ylim([-600, 900]);plt.yticks(np.arange(-600, 900, 200), fontsize=18)
        plt.axhline(0, linestyle='solid', color='black') 
        ...
        plt.yticks(np.arange(-600, 900, 200), fontsize=18);plt.xticks(np.arange(1, 8, 1), np.arange(1, 8, 1), fontsize=15)
        plt.xlim([2, 7.2]);plt.ylim([-600, 900])
        plt.tight_layout()
        plt.show()
...
def check_blinks(dm_blinks):
    ...
    for p, t, sdm in ops.split(dm_blinks.subject_id, dm_blinks.stim_color):
        dm_blinks.n_blinks[sdm] = srs.reduce(sdm.blinkstlist_imag, operation=count_nonnan) 
    ...
    x = np.array(dm_blinks.fixxlist_imag[dm_blinks.stim_color=='bright'])
    y = np.array(dm_blinks.fixylist_imag[dm_blinks.stim_color=='bright'])
    x = x.flatten()
    y = y.flatten()
    ...
    x = np.array(dm_blinks.fixxlist_imag[dm_blinks.stim_color=='dark'])
    y = np.array(dm_blinks.fixylist_imag[dm_blinks.stim_color=='dark'])
    x = x.flatten()
    y = y.flatten()
    plt.hexbin(x, y, gridsize=25)
    plt.axhline(500, color='white');plt.axvline(500, color='white')
...
print(f'Vividness: M = {dm_grouped.rating.mean}, STD = {dm_grouped.rating.std}, min = {dm_grouped.rating.min}, max = {dm_b.rating.max})')    
...
print(f'Dark: M = {np.round(dm_processed.rating[dm_processed.stim_color == "dark"].mean,2)}, SD = {np.round(dm_processed.rating[dm_processed.stim_color == "dark"].std, 2)}')
print(f'Bright: M = {np.round(dm_processed.rating[dm_processed.stim_color == "bright"].mean,2)}, SD = {np.round(dm_processed.rating[dm_processed.stim_color == "bright"].std, 2)}')
...
#m3 = lm_pupil(dm_grouped, formula='pupil_change ~ rating', re_formula='1 + stim_orientation', pupil_change=True, reml=True)
...
plt.title('Perception first (PF) version')
...
# Supp Correlations
plt.figure(figsize=(8,8))
plt.title('Perception-first version')
